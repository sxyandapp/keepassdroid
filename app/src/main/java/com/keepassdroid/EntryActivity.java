...
//		Notification notify = new Notification(R.drawable.notify, desc, System.currentTimeMillis());
		Notification.Builder bb = new Notification.Builder(this);
		bb.setSmallIcon(R.drawable.notify);
		bb.setTicker(desc);// 状态栏上显示
		bb.setContentTitle(getString(R.string.app_name));// 设置通知的标题
		bb.setContentText(desc);// 设置通知的内容
		bb.setWhen(System.currentTimeMillis());

		Intent intent = new Intent(intentText);
		PendingIntent pending = PendingIntent.getBroadcast(this, 0, intent, PendingIntent.FLAG_CANCEL_CURRENT);

		bb.setContentIntent(pending);

		Notification notify = bb.build();
