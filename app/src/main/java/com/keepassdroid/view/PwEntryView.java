...
public class PwEntryView extends ClickView implements View.OnClickListener{
	...
	protected PwEntryView(GroupBaseActivity act, PwEntry pw, int pos) {
		...
		Button cp= (Button) ev.findViewById(R.id.cp_password);
	...
	public void onClick(View v) {
		System.out.println("赋值密码到剪贴板");
        timeoutCopyToClipboard(mPw.getPassword());
	}

    private void timeoutCopyToClipboard(String text) {
        try {
            Util.copyToClipboard(App.INSTANCE, text);
            Toast.makeText(App.INSTANCE, "密码已复制到剪贴板", Toast.LENGTH_SHORT).show();
        } catch (SamsungClipboardException e) {
            System.out.println("不适用于三星手机");
        }
    }
